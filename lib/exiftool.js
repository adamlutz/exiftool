  ...
  exif.on("close", function () {
    ...
    else
    {
      ...
      response.forEach(function (responseLine) {
        var first_colon = responseLine.indexOf(':');
        var key = responseLine.substring(0, first_colon);
        var value = responseLine.substring(first_colon + 1, responseLine.length);

        var pieces = [key, value];

        //Is this a line with a meta data pair on it?
        if (pieces.length == 2 && pieces[0].trim() !== '')
        {
